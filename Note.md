...
1. 定义：以访问内存的方式访问文件数据
   ...
   - 操作系统将文件的某一部分（或整个文件）映射到进程的虚拟内存地址空间。
   - 实际数据仍驻留在磁盘上，但通过**页表**关联到内存地址。

   * 文件数据的读入、写出由操作系统自动完成
   * 当进程访问映射的内存区域时，若数据未加载到物理内存，会触发**缺页中断**，由操作系统自动从磁盘读取数据到内存（按需加载）。
...
3. 内存映射文件的优点：
   ...
   - **高效I/O**：
     - 避免了用户态与内核态之间的数据拷贝（如`read/write`需要缓冲区拷贝），减少CPU开销。
     - 利用操作系统的**页缓存**机制，自动预读或延迟写回，优化性能。
   - **随机访问**：
     - 直接通过指针操作文件内容，无需`lseek()`等定位操作，适合大文件或频繁随机访问的场景
   - **天然共享内存通信**
   - **懒加载**
